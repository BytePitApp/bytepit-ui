...
const ViewProblemBox: React.FC<ProblemProps> = ({ problem }) => {
        ...
        <>
            <div className="p-8 m-4 bg-gray-100 rounded-lg shadow-lg border border-gray-200">
                <h2 className="text-2xl text-primary font-bold mb-4">
                    ...
                    <span
                        className={`ml-2 px-2 py-1 rounded text-white ${
                            getDifficulty(problem.num_of_points) === "Easy"
                                ? "bg-green-500"
                                : getDifficulty(problem.num_of_points) === "Medium"
                                ? "bg-yellow-500"
                                : "bg-red-500"
                        }`}
                    >
                ...
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="p-2 rounded transition-colors duration-200 hover:bg-gray-200">
                        <Attribute
                            icon={problem.is_hidden ? faLock : faEyeSlash}
                            label="Hidden"
                            value={problem.is_hidden ? "True" : "False"}
                        />
                    </div>
                    <div className="p-2 rounded transition-colors duration-200 hover:bg-gray-200">
                        <span>Created by: </span>
                        <ProfileLink
                            profileUrl={`/profiles/organiser/${organiserUsername}`}
